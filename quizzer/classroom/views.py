...
def index(request):
    user = request.user
    #rendering courses the user has
...
def NewCourse(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
            ...
            #creating object
			Course.objects.create(picture=picture, title=title, description=description, category=category, modules=modules, user=user)
...
def EditCourse(request, course_id):
    ...
    if user != course.user:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                #overwriting with new data and save
                ...
                course.modules = form.cleaned_data.get('modules')
                course.save()
